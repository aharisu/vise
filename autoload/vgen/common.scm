...
(define (env-find-data-with-outside-lambda-count env symbol)
  (let1 symbol (get-symbol symbol)
    (let loop ((env env)
               (num-outside 0))
      (if-let1 d (assq-ref (@ env.symbols) symbol)
        (values d num-outside)
        (if (@ env.parent)
          (loop (@ env.parent) 
                (if (@ env.lambda-border?)
                  (+ num-outside 1)
                  num-outside))
          (values #f 0))))))

(define (env-find-data-with-outside-lambda? env symbol)
  (receive (d num-outside)
    (env-find-data-with-outside-lambda-count env symbol)
    (values d (not (zero? num-outside)))))
